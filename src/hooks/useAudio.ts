...
export const useAudio = (globalPlaylist: any[] = PLAYLIST) => {
  const [sound, setSound] = useState<Audio.Sound | null>(null);
  const [isPlaying, setIsPlaying] = useState(false);
  const [position, setPosition] = useState(0);
  const [duration, setDuration] = useState(0);
  const [currentIndex, setCurrentIndex] = useState(0); 
  const [currentQueue, setCurrentQueue] = useState<any[]>(globalPlaylist);
  const [originalQueue, setOriginalQueue] = useState<any[]>(globalPlaylist);
  ...
  // Sync queues when globalPlaylist populates from Firebase (only if not playing anything yet)
  useEffect(() => {
    if (globalPlaylist.length > 0 && currentQueue.length === 0) {
      setCurrentQueue(globalPlaylist);
      setOriginalQueue(globalPlaylist);
    }
  }, [globalPlaylist, currentQueue.length]);
  ...
  const handlePlayTrack = (index: number, newQueue: any[] = globalPlaylist, forceShuffle: boolean = false) => {
  ...
  return {
    ...
    handleSlidingStart, handleSlidingComplete, setPosition, handleVolumeChange, handleStopMusic
  };
};