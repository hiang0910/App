...
const ProfileScreen = ({
  currentUser, setCurrentScreen, actualLogout, handleBack, userPlaylists, handleCreatePlaylist, handleOpenTrackOptions, handleUpdateProfile, handleChangePassword, audio,
  dynamicPlaylist
}: any) => {
  ...
  useEffect(() => {
    ...
    const q = query(collection(db, "songs"), where("artistId", "==", currentUser.uid));
  ...
  const handlePostSong = async () => {
    ...
    try {
      const songData: any = {
        title: songTitle,
        artist: currentUser?.displayName || 'Nghệ sĩ',
      ...
      if (editingSongId) {
        ...
        songData.createdAt = Timestamp.now();
  ...
  const uploadFile = (uri: string) => {
    return new Promise<string>(async (resolve, reject) => {
      try {
        const xhr = new XMLHttpRequest();
        const formData = new FormData();
        
        // Cần fetch tệp để chuyển thành Blob thực sự (quan trọng tuyệt đối cho Web)
  ...
  const renderGenderOptions = () => (
    <View style={{ flexDirection: 'row', justifyContent: 'space-between', gap: 10 }}>
      {['Nam', 'Nữ', 'Khác'].map(g => (
        <TouchableOpacity
          key={g}
          style={[localStyles.editInput, { flex: 1, alignItems: 'center', justifyContent: 'center', borderColor: editGender === g ? '#0CD2D1' : '#2A3140' }]}
          onPress={() => setEditGender(g)}
        >
          <Text style={{ color: editGender === g ? '#0CD2D1' : '#6E7480', fontWeight: 'bold' }}>{g}</Text>
        </TouchableOpacity>
      ))}
    </View>
  );
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100 }}>
        ...
        {/* User Info Block */}
        <View style={localStyles.userInfoCard}>
          <TouchableOpacity
            style={{ position: 'absolute', top: 20, right: 20, zIndex: 10 }}
            onPress={() => setIsEditing(true)}
          >
            <Ionicons name="pencil" size={20} color="#0CD2D1" />
          </TouchableOpacity>
          ...
          <View style={{ flexDirection: 'row', gap: 10 }}>
            ...
            {isArtist && (
              <TouchableOpacity
                style={[localStyles.favoriteBtn, { backgroundColor: 'rgba(12, 210, 209, 0.1)', borderColor: '#0CD2D1' }]}
                onPress={() => setIsPostingSong(true)}
              >
                <Ionicons name="cloud-upload" size={20} color="#0CD2D1" style={{ marginRight: 8 }} />
                <Text style={[localStyles.favoriteBtnText, { color: '#0CD2D1' }]}>Đăng Nhạc</Text>
              </TouchableOpacity>
            )}
          </View>
        </View>
          ...
          <View style={localStyles.listCard}>
            ...
            <View style={{ flexDirection: 'row', gap: 15 }}>
              <TouchableOpacity style={localStyles.artistToolBtn} onPress={() => { resetPostingForm(); setIsPostingSong(true); }}>
                <Ionicons name="musical-notes-outline" size={30} color="#0CD2D1" />
                <Text style={localStyles.artistToolText}>Đăng Nhạc</Text>
              </TouchableOpacity>
              <TouchableOpacity style={localStyles.artistToolBtn} onPress={() => setIsCreatingAlbum(true)}>
            ...
            {mySongs.map((song) => (
              <View key={song.id} style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: 'rgba(255,255,255,0.03)', padding: 10, borderRadius: 12, marginBottom: 10 }}>
                ...
                <View style={{ flex: 1, marginLeft: 15 }}>
                  ...
                  <Text style={{ color: '#6E7480', fontSize: 12 }}>{song.genre} • {song.duration}</Text>
                </View>
                <View style={{ flexDirection: 'row', gap: 10 }}>
        ...
        <View style={localStyles.listCard}>
          ...
          <View style={localStyles.createPlaylistRow}>
            ...
            />
            <TouchableOpacity style={localStyles.createBtn} onPress={createNewPlaylist}>
          ...
          {userPlaylists && userPlaylists.map((pl: any) => (
            <View key={pl.id} style={localStyles.playlistRow}>
              <View style={localStyles.playlistIconWrapper}>
                <Ionicons name="musical-notes" size={24} color="#0CD2D1" />
              </View>
              <View style={{ flex: 1 }}>
        ...
        <View style={localStyles.modalOverlay}>
          <View style={localStyles.bottomModal}>
            <View style={localStyles.modalHeader}>
              <Text style={localStyles.modalTitle}>{editingSongId ? "Sửa Bài Hát" : "Đăng Bài Hát Mới"}</Text>
              <TouchableOpacity onPress={() => { setIsPostingSong(false); resetPostingForm(); }}>
                <Ionicons name="close" size={28} color="#FF6347" />
              </TouchableOpacity>
            </View>
            ...
            <View style={{ flexDirection: 'row', gap: 15 }}>
              <View style={{ flex: 1 }}>
                ...
                <TextInput style={localStyles.editInput} value={songGenre} onChangeText={setSongGenre} placeholder="Pop, Rock..." placeholderTextColor="#6E7480" />
              </View>
              <View style={{ flex: 1 }}>
      ...
      <Modal visible={isCreatingAlbum} transparent animationType="fade">
        <View style={localStyles.modalOverlay}>
          <View style={[localStyles.bottomModal, { borderRadius: 20, marginBottom: 'auto', marginTop: 'auto' }]}>
            <View style={localStyles.modalHeader}>
              <Text style={localStyles.modalTitle}>Tạo Album Mới</Text>
              <TouchableOpacity onPress={() => setIsCreatingAlbum(false)}>
                <Ionicons name="close" size={28} color="#FF6347" />
              </TouchableOpacity>
            </View>
            ...
            <Text style={localStyles.editLabel}>Ảnh bìa Album (URL)</Text>
            <TextInput style={localStyles.editInput} value={albumCover} onChangeText={setAlbumCover} placeholder="https://..." placeholderTextColor="#6E7480" />

            <TouchableOpacity style={[localStyles.saveBtn, { backgroundColor: '#FFD700' }]} onPress={handleCreateAlbum}>
              <Text style={{ color: '#000', fontWeight: 'bold' }}>TẠO ALBUM</Text>
            </TouchableOpacity>
          </View>
        </View>
      </Modal>
...
const localStyles = StyleSheet.create({
  ...
  createInput: { flex: 1, backgroundColor: 'rgba(255,255,255,0.05)', height: 50, borderRadius: 10, paddingHorizontal: 15, color: '#FFF' },
  ...
  editInput: { backgroundColor: '#151C2C', borderWidth: 1, borderColor: '#2A3140', color: '#FFF', paddingHorizontal: 15, height: 45, borderRadius: 8 },
